...
ui <- dashboardPage(
  dashboardHeader(
    title = "Trip Length Distributions - 2012 Household Travel Survey",
    titleWidth = 600,
  ...
  dashboardSidebar(
    selectInput("groupSampleSegment", "Sample Segment Group:",
                choices = c('All','Cache','Dixie','WFRC-MAG','Utah Other')),
    selectInput("groupModeTypeBroad", "Travel Mode:",
                choices = setNames(c(-1,1,2,3,4,5,6),c('All','Auto/truck/motorcycle','Transit','Walked/Wheelchair','Bicycle','Other','School bus (only independent child trips)'))),
    selectInput("groupTripType", "Trip Purpose:",
                choices = c('All','HBW','HBSch','HBShp','HBPb','HBO','NHBW','NHBNW')),
    selectInput("groupNumVehicles", "Household Number of Vehicles:",
                choices = setNames(c(-1,0,1,2,3,4,5),c('All','0 (no vehicles)','1 vehicle','2 vehicles','3 vehicles','4 vehicles','5 or more vehicles'))),
    selectInput("groupNumWorkers", "Household Number of Workers:",
                choices = setNames(c(-1,0,1,2,3,4,5,6),c('All','0','1','2','3','4','5','6'))),
...
server <- function(input, output, session) {
  ...
  shinyalert(
    text ="The 2012 household travel survey was designed and conducted for use in regional and statewide travel demand modeling. The sample size and frame is suitable for that purpose.
  ...
  observe({
    ...
    output$dataPlot <- renderPlot({
      ...
      filtered_data <- data %>%
        filter(groupRegionName == input$groupSampleSegment,
               groupWorkers == input$groupNumWorkers,
               groupVehicles == input$groupNumVehicles,
               groupTripPurpose == input$groupTripType,
               groupMainMode == input$groupModeTypeBroad,
    ...
    output$selectedInputs <- renderText({
      paste("\n",
            "Current Filter:",
            paste0("\u00A0\u00A0\u00A0Region Name: ", input$groupSampleSegment),
            paste0("\u00A0\u00A0\u00A0Travel Mode: ", input$groupModeTypeBroad),
            paste0("\u00A0\u00A0\u00A0Trip Type: ", input$groupTripType),
            paste0("\u00A0\u00A0\u00A0Number of Vehicles: ", input$groupNumVehicles),
            paste0("\u00A0\u00A0\u00A0Number of Workers: ", input$groupNumWorkers),
            sep = "\n")
    })
